...
主 Agent 按阶段读取对应模板，替换方括号字段后派发。文件路径和稳定 ID 优先于粘贴长文；subagent 可以沿路径读取必要上下文，但只能在声明的写入范围内行动。
...
Grill 是主 Agent 与用户之间的决策过程，不能让 subagent 代替用户回答。事实问题可以先用 Explore prompt 调查。

所有 subagent 返回：状态、读取与修改的文件、实际证据、未完成项、越界风险和建议下一步。报告不是事实；主 Agent 必须核对工作区和验证结果。
